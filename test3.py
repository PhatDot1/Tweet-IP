# i think this one reverts because the nft contract isnt being added to the story IP asset registry??
import os
import sys
import time
import uuid
...
MAX_SUPPLY              = 1000
ROYALTY_BPS             = 500
ROYALTY_RECEIVER        = None  # we'll set this below
...
def main():
    try:
        ...
        # use low-level registration_workflows to get accurate gas
        ...
        ip      = story.IPAsset.mint_and_register_ip(
            ...
            ip_metadata={},  # ignored when passing build_fn
            recipient=acct.address,
            allow_duplicates=True,
            tx_options={
                "from":     acct.address,
                "nonce":    nonce,
                "gas":      int(est_ip * 10),    # ← 10× estimate
                "gasPrice": w3.eth.gas_price,
            },
            build_fn=lambda opts: mint_fn.build_transaction(opts)
        )
        ipid     = ip.get("ip_id")
        token_id = ip.get("token_id")
        print("✅  IP Asset:", ipid)

        # ── ◾ INSPECT ON-CHAIN REGISTRY ◾ ──────────────────────────────────
        REGISTRY_ABI = [
            {
                "inputs":[
        ...
        registry   = w3.eth.contract(
            address=Web3.to_checksum_address(IP_REGISTRY_ADDRESS),
            abi=REGISTRY_ABI
        )
        chain_id   = w3.eth.chain_id
        onchain_ip = registry.functions.ipId(chain_id, child, token_id).call()
        is_reg     = registry.functions.isRegistered(onchain_ip).call()
        print("👀 registry.ipId          →", onchain_ip)
        print("👀 registry.isRegistered  →", is_reg)
